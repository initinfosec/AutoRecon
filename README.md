...
A number of updates were made by initinfosec to the main autorecon script. These include

* creation of a setup script wrapper to [hopefully] reliably take care of installation of all requirements
* setup script includes optional installation of 'etc' tools, extended tools which can be used but are not required for the main script. However, some commands (mostly manual ones not automatically run) might fail without the etc tools installed. Currently this includes: seclists, dirsearch, golang, ffuf, and enum4linux-ng
* enulin4linux-ng added as a standard comman run alongside enum4linux in case enum4linux-ng can provide info where the original script might fail or fall short
* ffuf added as a manual command option

Please find installation instructions for utilizing the script below.
